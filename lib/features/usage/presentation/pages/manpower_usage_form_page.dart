...
class _ManpowerUsageFormPageState extends State<ManpowerUsageFormPage> {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      if (doc != null) {
        ...
        final rawItems = data['manpower_usage'] as List? ?? [];
        _items = rawItems
        ...
        if (_siteEngineer != null && _siteEngineer!.isNotEmpty) {
          try {
            ...
            final name = empData['employee_name']?.toString() ?? _siteEngineer!;
            _siteEngineerController.text = name;
          } catch (_) {}
        }
        ...
        // Defaults for new manpower usage
  ...
  int get _totalWorkers {
    return _items.fold(0, (sum, item) {
      final workers =
          int.tryParse(item['no_of_workers']?.toString() ?? '0') ?? 0;
      return sum + workers;
    });
  }
  ...
  Future<void> _showLinkSearch(
    ...
    Future<List<LinkOptionEntity>> Function(String query)? customFetcher,
  }) async {
  ...
  Future<void> _saveDocument() async {
    ...
    try {
      if (_document == null) {
        ...
        if (serverName != null) {
          final merged = Map<String, dynamic>.from(payload)
            ..['name'] = serverName;
          try {
            await sdk.api.document.submitDocument('Manpower Usage', serverName);
            merged['docstatus'] = 1;
          } catch (e) {
            merged['docstatus'] = 0;
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Manpower Usage saved successfully')),
        );
            ...
            _buildCard(
              ...
              children: [
                ...
                _buildTextField(
                  ...
                  onTap: () => _showLinkSearch('Employee', (val) async {
                    setState(() {
                      _siteEngineer = val;
                    ...
                    try {
                      ...
                      final name = empData['employee_name']?.toString() ?? val;
                      setState(() {
                        _siteEngineerController.text = name;
                      });
                    } catch (_) {}
                  ...
                  if (_items.isEmpty)
                    ...
                    ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final skill = item['skill_type']?.toString() ?? '';
                        final workers =
                            int.tryParse(
                              item['no_of_workers']?.toString() ?? '1',
                            ) ??
                            1;
                        final hours =
                            double.tryParse(item['hours']?.toString() ?? '8') ??
                            8.0;
                        final amount =
                            double.tryParse(
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        '$workers workers x ${hours.toStringAsFixed(1)} hrs',
                                        style: const TextStyle(
                                          fontSize: 13,
                                          color: Colors.black87,
                                        ),
...
class _ItemEditorBottomSheetState extends State<_ItemEditorBottomSheet> {
  ...
  late TextEditingController _skillTypeController;
  late TextEditingController _timeInController;
  late TextEditingController _timeOutController;
  late TextEditingController _presentyController;
  late TextEditingController _hoursController;
  late TextEditingController _totalPresentyController;
  ...
  final List<String> _skillTypeOptions = ['Skilled', 'Unskilled'];

  Future<void> _fetchChildMeta() async {
    try {
      final sdk = sl<FrappeSDK>();
  ...
  void _setMetaOptions(dynamic meta) {
    try {
      debugPrint(
        'Manpower Usage Details Fields: ${meta.fields.map((f) => '${f.fieldname} (${f.fieldtype})').toList()}',
      );
      final field = meta.fields.firstWhere((f) => f.fieldname == 'skill_type');
      debugPrint('Found skill_type field: options = ${field.options}');
      if (field.options != null) {
        final String optionsStr = field.options!.toString();
        final List<String> options = optionsStr
            .split('\n')
            .map<String>((s) => s.trim())
            .where((s) => s.isNotEmpty)
            .toList();
        debugPrint('Parsed skill_type options: $options');
        if (options.isNotEmpty) {
          setState(() {
            final filtered = options
                .where((opt) => opt == 'Skilled' || opt == 'Unskilled')
                .toList();
            _skillTypeOptions.clear();
            if (filtered.isEmpty) {
              _skillTypeOptions.addAll(['Skilled', 'Unskilled']);
            } else {
              _skillTypeOptions.addAll(filtered);
            }
          });
        }
      }
    } catch (e, stack) {
      debugPrint('Error in _setMetaOptions: $e\n$stack');
    }
  }
  ...
  void initState() {
    ...
    _skillTypeController = TextEditingController(
      text: _item['skill_type']?.toString(),
    );
    _timeInController = TextEditingController(
      text: _item['time_in']?.toString(),
    );
    _timeOutController = TextEditingController(
      text: _item['time_out']?.toString(),
    );
    _presentyController = TextEditingController(
      text: _item['presenty']?.toString() ?? '1.0',
    );
    _hoursController = TextEditingController(
      text: _item['hours']?.toString() ?? '8.0',
    );
    _totalPresentyController = TextEditingController(
      text: _item['total_presenty']?.toString() ?? '1.0',
    );
    ...
    _noOfWorkersController = TextEditingController(
      text: _item['no_of_workers']?.toString() ?? '1',
    );
  ...
  void dispose() {
    ...
    _skillTypeController.dispose();
    _timeInController.dispose();
    _timeOutController.dispose();
    _presentyController.dispose();
    _hoursController.dispose();
    _totalPresentyController.dispose();
  ...
  void _calculateAmount() {
    final workers = double.tryParse(_noOfWorkersController.text) ?? 1.0;
    final hours = double.tryParse(_hoursController.text) ?? 8.0;
    final rate = double.tryParse(_rateController.text) ?? 0.0;
    final total = workers * hours * rate;
    setState(() {
      _item['no_of_workers'] = int.tryParse(_noOfWorkersController.text) ?? 1;
      _item['hours'] = hours;
      _item['rate'] = rate;
      _item['amount'] = total;
    ...
    return Container(
      ...
      child: Scaffold(
        ...
        children: [
                    ...
                    children: [
                      GestureDetector(
                        onTap: () => widget.showLinkSearch('Contractor', (val) {
                          setState(() {
                            if (_item['contractor'] != val) {
                              _item['contractor'] = val;
                              _contractorController.text = val;
                              // Reset item selection if contractor changed
                            ...
                            customFetcher: (String query) async {
                              ...
                              if (contractor.isNotEmpty) {
                                try {
                                  ...
                                  final uri = Uri.parse(
                                    '$baseUrl/api/method/frappe.desk.search.search_link'
                                    '?txt=${Uri.encodeComponent(query)}'
                                    '&doctype=Item'
                                    '&ignore_user_permissions=0'
                                    '&reference_doctype=Manpower%20Usage%20Details'
                                    '&page_length=20'
                                    '&link_fieldname=equipment_item'
                                    '&query=quantbit_construction_management.site_diary.doctype.manpower_usage.manpower_usage.get_contractor_manpower_items'
                                    '&filters=${Uri.encodeComponent(filtersJson)}',
                                  );
                                  final response = await sdk.api.rest.client
                                      .get(
                                        uri,
                                        headers: sdk.api.requestHeaders,
                                      );
                          ...
                          Expanded(
                            child: GestureDetector(
                              onTap: () async {
                                ...
                                if (time != null) {
                                  final hr = time.hour.toString().padLeft(
                                    2,
                                    '0',
                                  );
                                  ...
                                  final formatted = '$hr:$min:00';
                                  setState(() {
                                    _item['time_in'] = formatted;
                      ...
                      Row(
                        children: [
                          Expanded(
                            child: _buildTextField(
                              label: 'Presenty',
                              controller: _presentyController,
                              placeholder: '1.0',
                              keyboardType:
                      ...
                      _buildDropdownField(
                        label: 'Skill Type',
                        value: _item['skill_type']?.toString(),
                        items: _skillTypeOptions,
                        onChanged: (val) {
                          setState(() {
                            _item['skill_type'] = val;
                            _skillTypeController.text = val ?? '';
                          });
                        },
                      ),
                      SizedBox(height: sizeContextOf(context, 12)),
          ...
          ElevatedButton(
            onPressed: () {
              // Validation: Must select at least a Stage and a terminal non-group Task, or whatever subtask is selected
              // In this implementation, the last selected non-empty task level is the active task.
              // Let's find the deepest selected task:
              String activeTask = '';
              if (_taskController.text.isNotEmpty) {
              ...
              _item['no_of_workers'] =
                  int.tryParse(_noOfWorkersController.text) ?? 1;
              _item['hours'] = double.tryParse(_hoursController.text) ?? 8.0;
              _item['rate'] = double.tryParse(_rateController.text) ?? 0.0;
              _item['amount'] = double.tryParse(_amountController.text) ?? 0.0;
              _item['skill_type'] = _skillTypeController.text;
              _item['contractor'] = _contractorController.text;
              // _item['equipment_item'] is already updated in state
              _item['uom'] = _uomController.text;
              _item['time_in'] = _timeInController.text;
              _item['time_out'] = _timeOutController.text;
              _item['presenty'] =
                  double.tryParse(_presentyController.text) ?? 1.0;
