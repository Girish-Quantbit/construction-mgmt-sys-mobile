...
class EquipmentUsageDetailPage extends StatelessWidget {
    ...
    return BlocProvider(
      ...
      child: Scaffold(
                ...
                return IconButton(
                  ...
                  onPressed: () async {
                    final result = await Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => EquipmentUsageFormPage(
                          usageName: usageName,
                        ),
                      ),
                    );
        ...
        body: BlocBuilder<EquipmentUsageBloc, EquipmentUsageState>(
          builder: (context, state) {
            if (state.detailStatus == EquipmentUsageStatus.loading) {
              return const Center(child: CircularProgressIndicator());
            }

            if (state.detailStatus == EquipmentUsageStatus.failure) {
              return Center(
                child: Column(
            ...
            return FutureBuilder<DocTypeMeta>(
              future: sl<FrappeSDK>().meta.getMeta('Equipment Usage'),
  ...
  Widget _buildGeneralInfoCard(EquipmentUsage usage, DocTypeMeta? meta) {
  ...
  Widget _buildItemsCard(BuildContext context, EquipmentUsage usage) {
    return Card(
      elevation: 0,
      color: AppColors.surfaceContainerLowest,
        ...
        children: [
          SingleChildScrollView(
            ...
            child: DataTable(
              ...
              rows: usage.equipmentUsageDetails.map<DataRow>((item) {
                return DataRow(
                  cells: [
          ...
          if (usage.equipmentUsageDetails.isEmpty)
            const Padding(
