...
abstract class ManpowerUsageRemoteDataSource {
  Future<List<dynamic>> getManpowerUsages({
    int page = 1,
    int pageSize = 20,
    String? search,
    String? status,
  ...
  Future<Map<String, dynamic>> getManpowerUsageDetails(String name);
...
class ManpowerUsageRemoteDataSourceImpl implements ManpowerUsageRemoteDataSource {
  final FrappeSDK sdk;

  ManpowerUsageRemoteDataSourceImpl({required this.sdk});
  ...
  Future<List<dynamic>> getManpowerUsages({
    ...
    if (status != null && status.isNotEmpty) {
      filters.add(['Manpower Usage', 'status', '=', status]);
    }
    if (project != null && project.isNotEmpty) {
      filters.add(['Manpower Usage', 'project', '=', project]);
    }
  ...
  @override
  Future<Map<String, dynamic>> getManpowerUsageDetails(String name) async {
    return sdk.api.doctype.getByName('Manpower Usage', name);
  }

  @override
  Future<DocTypeMeta> getMeta() async {
    return sdk.meta.getMeta('Manpower Usage', forceRefresh: true);
  }
  ...
  Future<Map<String, dynamic>?> loadDocument(String? usageName) async {
    ...
    try {
      final serverData = await sdk.api.doctype.getByName('Manpower Usage', usageName);
  ...
  Future<String> saveDocument({
    ...
    if (existingServerId == null) {
      final result = await sdk.api.document.createDocument('Manpower Usage', payload);
      final serverName =
